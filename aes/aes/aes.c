...
//    printf("n = %d, row = %d, col = %d, c = %d \n", index, row, col, S[row][col]);
    return S[row][col];
}
...
static int getWordFromStr(char *str) {
    ...
    int r = one | two | three | four;
    printf("one = %d, two = %d, three = %d, four = %d \n", one, two, three, four);
    printf("getWordFromStr : %d \n", r);
    return r;
}
...
void printState(int pArray[4][4]) {
    printf("[");
    for (int i = 0; i < 4; i ++) {
        printf("[");
        for (int j = 0; j < 4; j++) {
...
void aes(char *p, int plen, char *key){
    ...
    for(int k = 0; k < plen; k += 16) { 
        ...
        for(int i = 1; i < 10; i++){//前9轮
            ...
            printf("before shiftRows, %d : ", i);
            printState(pArray);
            shiftRows(pArray);//行移位
            printf("before mixColum, %d : ", i);
            printState(pArray);
            mixColumns(pArray);//列混合
            printf("after mixColum, %d : ", i);
            printState(pArray);
            addRoundKey(pArray, i);
            printf("after addRoundKey, %d : ", i);
            printState(pArray);
        }
